        ...
        for (move, direction) in movements:
            ...
            if 0 <= next_row < size and 0 <= next_column < size: 
                ...
                if space[next_row][next_column] != 'H' and next_pos not in parent:
                    states_explored += 1
                    # Costo de llegar al nodo que se acaba de llegar
                    new_cost = current_node_cost + movement_costs[direction]
                    # Costo de llegar más la heurística
                    next_columncost = new_cost + env.manhattan_distance(next_pos, goal)
